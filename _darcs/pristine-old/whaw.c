...
get_viewarea(int screen_num, int *x, int *y, int *w, int *h)
{
        ...
        *h = s->height;
        ...
        int num = get_property(dpy,DefaultRootWindow(dpy), XA__NET_CLIENT_LIST, XA_WINDOW, clients, sizeof(long)*MAX_CLIENTS, 0, 0);
        //printf("Num clients: %i\n", num/ sizeof(long));
        for(int i = 0; i < num / sizeof(long); i++) {
                //printf("client: 0x%lx\n", (unsigned long)clients[i]);
                long net_strut[12];
                memset(net_strut,0,sizeof(net_strut));
                int n = get_property(dpy,clients[i], XA__NET_WM_STRUT_PARTIAL, XA_CARDINAL, net_strut, sizeof(long)*12, 0,0);
                if(!n)
                ...
                if(n/sizeof(long) == 4) {
                      ...
                      strut_bottom += net_strut[3];
                        ...
                        //printf("long strut\n");
                        if(IN(net_strut[4],*y,*h) || IN(net_strut[5], *y, *h))
                              strut_left += net_strut[0];
                        if(IN(net_strut[6],*y,*h) || IN(net_strut[7], *y, *h))
                              strut_right += net_strut[1];
                        if(IN(net_strut[8],*x,*w) || IN(net_strut[9], *x, *w))
                              strut_top += net_strut[2];
                        if(IN(net_strut[10],*x,*w) || IN(net_strut[11], *x, *w))
                              strut_bottom += net_strut[3];
                }
        }
        *x += strut_left;
...
move_window(Display *dpy, Window w, int x, int y, int width, int height)
{
        ...
        int ret = get_property(dpy,w, XA__NET_FRAME_EXTENTS, XA_CARDINAL, extents, sizeof(long)*4, sizeof(long)*4, 0);
        ...
        while(1) {
                ...
                if(process_event(&ev, &event, event_state)) {
                        for(;event;event = event->next) {
                                ...
                                case button_press:
                                        assert(!in_drag);
                                        if(event->button == 2 || ((event->button == 3) && !head)) {
                        ...
                        case ConfigureNotify:
                                ...
                                break;
                        case EnterNotify:
                                if(ev.xcrossing.window != w || in_drag)
                                        break;
